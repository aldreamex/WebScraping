"""Simple users request and response parsing and object representation

This module contains objects representing simple users requests
...
OpenID providers.
  ...
  2. The OpenID provider extracts the simple users request from
...
@var sreg_uri: The preferred URI to use for the simple users
...
# URI in the draft specification for simple users 1.1
...
class SRegNamespaceError(ValueError):
    """The simple users namespace was not found and could not
...
def getSRegNS(message):
    ...
    for sreg_ns_uri in [ns_uri_1_1, ns_uri_1_0]:
        ...
        try:
            ...
            # defined for something other than simple users
...
class SRegRequest(Extension):
    """An object to hold the state of a simple users request.
    ...
    def __init__(self,
                 ...
                 policy_url=None,
                 sreg_ns_uri=ns_uri):
        """Initialize an empty simple users request"""
    ...
    def fromOpenIDRequest(cls, request):
        """Create a simple users request that contains the
        ...
        @returns: The newly created simple users request
    ...
    def parseExtensionArgs(self, args, strict=False):
        ...
        @param args: The unqualified simple users arguments
        @type args: {str:str}

        @param strict: Whether requests with fields that are not
            defined in the simple users specification should be
    ...
    def allRequestedFields(self):
        """A list of all of the simple users fields that were
    ...
    def wereFieldsRequested(self):
        """Have any simple users fields been requested?
    ...
    def requestField(self, field_name, required=False, strict=False):
        ...
        @param field_name: the unqualified simple users field name
        @type field_name: str
        ...
        @raise ValueError: when the field requested is not a simple
            users field or strict is set and the field was
    ...
    def requestFields(self, field_names, required=False, strict=False):
        ...
        @param field_names: The simple users data fields to request
        ...
        @raise ValueError: when a field requested is not a simple
            users field or strict is set and a field was
            requested more than once
        """
    ...
    def getExtensionArgs(self):
        """Get a dictionary of unqualified simple users
...
class SRegResponse(Extension):
    """Represents the data returned in a simple users response
    ...
    @ivar data: The simple users data, keyed by the unqualified
        simple users name of the field (i.e. nickname is keyed
        by C{'nickname'})

    @ivar ns_uri: The URI under which the simple users data was
    ...
    def extractResponse(cls, request, data):
        ...
        users values and create a C{L{SRegResponse}}
        object containing that data.

        @param request: The simple users request object
        @type request: SRegRequest

        @param data: The simple users data for this
            response, as a dictionary from unqualified simple
            users field name to string (unicode) value. For
            instance, the nickname should be stored under the key
            'nickname'.
        @type data: {str:str}

        @returns: a simple users response object
        @rtype: SRegResponse
        """
        self = cls()
    ...
    def fromSuccessResponse(cls, success_response, signed_only=True):
        ...
        @rtype: SRegResponse
        @returns: A simple users response containing the data
    ...
    def getExtensionArgs(self):
        """Get the fields to put in the simple users namespace
    ...
    def get(self, field_name, default=None):
        ...
        defined by the simple users specification"""
        checkFieldName(field_name)
        return self.data.get(field_name, default)

    def items(self):
        """All of the data values in this simple users response
        """
